...
METRICS_SCHEMA = {
    ...
    "properties": {
        "metric": {"type": "string", "description": "The name of the timeseries."},
        "type": {
            "type": "integer",
            "enum": [3],
...
LOGS_SCHEMA = {
    ...
    "properties": {
        ...
        "ddtags": {"type": "string", "description": "Tags associated with your logs."},
        "timestamp": {
            "type": "integer",
        ...
        "data": {
            ...
            "properties": {
                ...
                "attributes": {
                    ...
                    "properties": {
                        "category": {"type": "string", "enum": ["alert"]},
...
def getThirdPartySinkConfigFromSparkConfig(spark_conf):
    ...
    if not all([metrics_endpoint, logs_endpoint, events_endpoint]):
        ...
        if events_endpoint is None:
            events_endpoint = (
                f"https://event-management-intake.{host_name}/api/v2/events"
            )
...
def enforce_schema(data, schema, path="root"):
    ...
    if isinstance(schema_type, list):
        ...
        for allowed_type in schema_type:
            ...
            raise ValueError(
                f"Value at {path} does not match any allowed types: {schema_type}"
            )
...
class HTTPClient:
    ...
    def _make_request_with_retry(
        ...
        try:
            ...
            if response.status_code >= 400 and response.status_code < 500:
                logging.warning(
                    f"Ignoring client-side error for URL: {url}, headers: {str(headers)}, Payload: {payload.decode('utf-8')}, Response: {response.text}"
                )
            else:
    ...
    def post(self, http_request_specs_df) -> None:
        ...
        for idx, row in enumerate(rows, 1):
            try:
                logging.info(
                    f"[HTTPClient] Sending request {idx}/{total_requests} to {row.endpoint}"
                )
                ...
                success_count += 1
                logging.info(
                    f"[HTTPClient] Successfully sent request {idx}/{total_requests}"
                )
            except Exception as e:
                failure_count += 1
                logging.error(
                    f"[HTTPClient] Failed to send request {idx}/{total_requests}: {str(e)}"
                )
                continue  # Continue with other requests regardless of success/failure

        logging.info(
            f"[HTTPClient] Completed sending requests: {success_count} succeeded, {failure_count} failed out of {total_requests} total"
        )
...
class DatadogEventsConverter:
    ...
    def create_event(
        ...
        event = {
            "data": {
                ...
                "attributes": {
                    ...
                    "attributes": {
                        "status": status,
                        "custom": {"source": SOURCE_NAME, "service": SERVICE_NAME},
                    },
                },
            }
        }
        ...
        # Add additional attributes if provided
        if additional_attributes:
            event["data"]["attributes"]["attributes"]["custom"].update(
    ...
    def create_http_requests_spec(
        ...
        return (
            df.withColumn("events", regexp_replace(col("events"), "\n", ""))
...
def convert_row_to_pipeline_metrics(row):
    ...
    end_time = getattr(row, "end_time", None) or datetime.now(timezone.utc).replace(
        tzinfo=None
    )
...
def register_sink_for_pipeline_events():
    ...
    def send_pipeline_status_to_3p_monitoring(batch_df, batch_id):
        ...
        logging.info(
            f"[Pipeline Events] Processing batch {batch_id} with {input_count} rows"
        )
        ...
        logging.info(
            f"[Pipeline Events] Converted {events_count} events from {input_count} input rows"
        )

        if events_count == 0:
            logging.info(
                f"[Pipeline Events] Skipping batch {batch_id} - no events to send"
            )
            return
        ...
        logging.info(
            f"[Pipeline Events] Sending {request_count} HTTP requests for batch {batch_id}"
        )
...
def register_sink_for_errors():
    ...
    def send_errors_to_3p_monitoring(batch_df, batch_id):
        ...
        logging.info(
            f"[Errors] Converted {logs_count} error logs from {input_count} input rows"
        )
        ...
        request_count = http_request_spec.count()
        logging.info(
            f"[Errors] Sending {request_count} HTTP requests for batch {batch_id}"
        )
        getClient(_global_config).post(http_request_spec)
    ...
    def send_errors_to_sink():
        return (
            ...
            .filter("error_message IS NOT NULL OR level = 'ERROR'")
        )
...
def register_sink_for_pipeline_metrics():
    ...
    def send_pipeline_metrics_to_3p_monitoring(batch_df, batch_id):
        input_count = batch_df.count()
        logging.info(
            f"[Pipeline Metrics] Processing batch {batch_id} with {input_count} rows"
        )
        ...
        logging.info(
            f"[Pipeline Metrics] Converted {metrics_count} metrics from {input_count} input rows"
        )

        if metrics_count == 0:
            logging.info(
                f"[Pipeline Metrics] Skipping batch {batch_id} - no metrics to send"
            )
            return
        ...
        request_count = http_request_spec.count()
        logging.info(
            f"[Pipeline Metrics] Sending {request_count} HTTP requests for batch {batch_id}"
        )
        getClient(_global_config).post(http_request_spec)
...
def register_sink_for_table_metrics():
    ...
    def send_table_metrics_to_3p_monitoring(batch_df, batch_id):
        input_count = batch_df.count()
        logging.info(
            f"[Table Metrics] Processing batch {batch_id} with {input_count} rows"
        )
        ...
        metrics_count = metrics_df.count()
        logging.info(
            f"[Table Metrics] Converted {metrics_count} metrics from {input_count} input rows"
        )

        if metrics_count == 0:
            logging.info(
                f"[Table Metrics] Skipping batch {batch_id} - no metrics to send"
            )
            return
        ...
        request_count = http_request_spec.count()
        logging.info(
            f"[Table Metrics] Sending {request_count} HTTP requests for batch {batch_id}"
        )
        getClient(_global_config).post(http_request_spec)
