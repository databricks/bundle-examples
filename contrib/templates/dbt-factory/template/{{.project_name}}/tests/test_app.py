...
def test_main_notebook_mode_auto_copies_runner_notebook_next_to_spec(
    ...
    assert copied_notebook.exists(), (
        "runner notebook should have been copied next to the job spec"
    )
    assert "dbtRunner" in copied_notebook.read_text(), (
        "copied file should be the packaged runner"
    )
...
def test_main_notebook_mode_auto_copies_runner_notebook_to_project_root(
    monkeypatch, tmp_path
):
    ...
    copied_notebook = tmp_path / "run_dbt_command.py"
    assert copied_notebook.exists(), (
        "runner should have been copied to the project root (one level up from the spec)"
    )
    assert not (spec_dir / "run_dbt_command.py").exists(), (
        "runner should NOT be copied next to the spec in this case"
    )
...
def test_main_notebook_mode(monkeypatch):
    ...
    dbt_manifest_path = BASE_PATH + "/test_data/manifest.json"
    input_job_spec_path = BASE_PATH + "/test_data/job_definition_template.yaml"
    expected_job_definition_path = (
        BASE_PATH + "/test_data/job_definition_notebook_default.yaml"
    )
...
def test_job_cluster_key_alone_parses(monkeypatch):
    monkeypatch.setattr(
        "sys.argv", ["main.py", *REQUIRED_ARGS, "--job-cluster-key", "foo"]
    )
    args = parse_args()
...
def test_environment_key_alone_parses(monkeypatch):
    monkeypatch.setattr(
        "sys.argv", ["main.py", *REQUIRED_ARGS, "--environment-key", "Default"]
    )
    args = parse_args()
...
def update_spec(
    ...
    # Update job name
    spec["resources"]["jobs"][new_job_name] = spec["resources"]["jobs"].pop(
        "dbt_sql_job"
    )
        ...
        for command in task["dbt_task"]["commands"]:
            if "--target dev" in command:
                command = command.replace(
                    "--target dev", f"--target dev {extra_dbt_command_options}"
                )
