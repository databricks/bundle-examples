...
class MonitoringEtlPipeline:
    ...
    def __init__(self, conf: Configuration, spark: SparkSession):
        ...
        self.register_delta_event_logs_from_pipelines_comma_list(
            self.conf.directly_monitored_pipeline_ids
        )
        self.register_delta_event_logs_from_pipelines_by_tags(
            self.conf.directly_monitored_pipeline_tags
        )
        self.register_imported_logs_tables_from_comma_list(
            self.conf.imported_event_log_tables, spark
        )
    ...
    def register_delta_event_logs_for_one_pipeline(self, pipeline_id: str):
        ...
        event_log_source = (
            f"`{event_log_info['catalog']}`.`{event_log_info['schema']}`.`{event_log_info['name']}`"
            if "name" in event_log_info
        ...
        self.pipeline_infos[pipeline_id] = PipelineInfo(
    ...
    def register_delta_event_logs_from_pipelines_comma_list(
        self, pipelines_comma_list: str
    ):
        """
        ...
        """
        self.register_delta_event_logs_for_pipelines(
    ...
    def register_delta_event_logs_from_pipelines_by_tags(self, tags_str: str):
        ...
        if not pipeline_ids:
            ...
            self.log.info(
                f"Found {len(pipeline_ids)} pipeline(s) matching tags, registering for monitoring"
            )
            self.register_delta_event_logs_for_pipelines(pipeline_ids)

    def register_one_imported_logs_table(
        self, imported_logs_table: str, spark: SparkSession
    ):
        """
        ...
        if len(imported_logs_table.split(".")) < 3:
            ...
            imported_logs_table = (
                f"`{self.conf.monitoring_catalog}`.`{self.conf.monitoring_schema}`.`{imported_logs_table}`"
                if imported_logs_table[0] != "`"
                else f"`{self.conf.monitoring_catalog}`.`{self.conf.monitoring_schema}`.{imported_logs_table}"
            )

        self.log.info(
            f"Detecting pipelines in imported logs table log {imported_logs_table} ..."
        )
        self.imported_event_log_tables.append(imported_logs_table)
        other_pipeline_ids = [
            r.pipeline_id
            for r in spark.sql(
                f"SELECT DISTINCT origin.pipeline_id FROM {imported_logs_table}"
            ).collect()
        ]
    ...
    def register_imported_logs_tables(
        self, imported_logs_tables: Iterable[str], spark: SparkSession
    ):
        """
    ...
    def register_imported_logs_tables_from_comma_list(
        self, imported_logs_tables_comma_list: str, spark: SparkSession
    ):
        """
        Detects all pipelines in a comma-separated of imported logs table
        """
        self.register_imported_logs_tables(
            parse_comma_separated_list(imported_logs_tables_comma_list), spark
        )

    def register_monitored_pipelines(self, spark: SparkSession):
        @dlt.table(
            name=MONITORED_PIPELINES.name,
    ...
    def register_event_log_source_views(self, spark: SparkSession) -> Dict[str, str]:
        ...
        def create_event_log_source_view(event_log_source: str) -> str:
            ...
            print(
                f"Defining source view {view_name} for event log source {event_log_source}"
            )
    ...
    def transfom_and_append_event_log_sources(
        self, target: str, flow_prefix: str, append_def: Callable[[str], DataFrame]
    ):
        """
        ...
        def process_el_source(el_source: str):
            ...
            print(
                f"Defining event log flow {flow_name} from {log_source} into {target}"
            )
    ...
    def register_created_pipeline_runs(self, spark: SparkSession):
        ...
        dlt.create_streaming_table(
            name=Constants.created_pipeline_runs,
    ...
    def register_event_logs_bronze(self, spark: SparkSession):
        ...
        def qualify_table_name_if_needed(
            table_name: str, default_catalog: str, default_schema: str
        ) -> str:
            """
            ...
            if (
                table_name is None
                or default_catalog is None
                or table_name.find(".") >= 0
            ):
                return table_name
        ...
        self.transfom_and_append_event_log_sources(
            target="flow_targets", flow_prefix="ft", append_def=append_to_flow_targets
        )

        dlt.create_streaming_table(
            name=EVENT_LOGS_BRONZE.name,
    ...
    def register_pipeline_run_status(self, spark: SparkSession):
      ...
      """)
        ...
        dlt.create_streaming_table(
            name=PIPELINE_RUNS_STATUS.name,
            cluster_by=["pipeline_id", "pipeline_run_id"],
    ...
    def register_metric_pipeline_hourly_error_rate(self, spark: SparkSession):
        @dlt.table(
            name=METRIC_PIPELINE_HOURLY_ERROR_RATE.name,
    ...
    def register_pipeline_status(self, spark: SparkSession):
        ...
        dlt.create_streaming_table(
            name=PIPELINES_STATUS_SILVER.name,
            cluster_by=["pipeline_id"],
    ...
    def register_events_table_metrics(self, spark: SparkSession):
        @dlt.table(
            name=EVENTS_TABLE_METRICS.name,
    ...
    def register_table_status_per_pipeline_run(self, spark: SparkSession):
        dlt.create_streaming_table(
            name=TABLE_STATUS_PER_PIPELINE_RUN.name,
    ...
    def register_table_status(self, spark: SparkSession):
        ...
        silver_latest_changes_source_view_name = (
            f"{silver_table_name}_latest_changes_source"
        )
    ...
    def register_table_expectation_checks(self, spark: SparkSession):
        @dlt.table(
            name=TABLE_EVENTS_EXPECTATION_CHECKS.name,
