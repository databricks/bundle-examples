...
has been trimmed to the single path this example uses — serverless compute with notebook tasks —
and returns a ``Resources`` object for PyDABs.
"""
...
def _dbt_databricks_dependency() -> str:
    ...
    """
    try:
        return f"dbt-databricks=={version('dbt-databricks')}"
...
def _write_serverless_environment_file() -> None:
    ...
    workspace, where the generated job references it via base_environment."""
    with open(SERVERLESS_ENV_FILE, "w") as f:
        yaml.safe_dump(_serverless_environment_spec(), f, sort_keys=False)
