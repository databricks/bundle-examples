...
This module is the only integration glue; the vendored core under ``src/databricks_dbt_factory``
is unchanged. It mirrors the wiring in ``databricks_dbt_factory.main`` but is fixed to the
options this example exposes (serverless + notebook task type) and returns a ``Resources``
object instead of writing a job-spec YAML file.
"""
...
# dbt-databricks version installed into the serverless environment at runtime.
DBT_DATABRICKS_DEPENDENCY = "dbt-databricks>=1.8.0,<2.0.0"
...
def _build_tasks(target: str) -> list[dict]:
    ...
    factory = DbtFactory(handler, task_factories, bundle_tests=BUNDLE_TESTS)
    manifest = handler.read_dbt_manifest(MANIFEST_PATH)
            ...
            "environments": [
                {
                    ...
                    "spec": {
                        ...
                        "dependencies": [DBT_DATABRICKS_DEPENDENCY],
                    },
                }
            ],
        }
...
def load_resources(bundle: Bundle) -> Resources:
    ...
    Reads the dbt manifest and registers the generated job. After deployment this function is
    not used.
    """
    resources = Resources()
