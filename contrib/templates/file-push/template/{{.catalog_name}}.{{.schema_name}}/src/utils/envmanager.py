...
def get_config() -> dict:
    json_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "configs", "environment.json"
    )
    if not os.path.exists(json_path):
        raise RuntimeError(
            f"Missing environment file: {json_path}. This should be run in a workspace, not locally. And make sure to run `databricks bundle run configuration_job` at least once."
        )
...
def has_default_storage() -> bool:
    ...
    return bool(
        storage_root
        or storage_location
        or props.get("defaultManagedLocation")
        or props.get("delta.defaultLocation")
    )
