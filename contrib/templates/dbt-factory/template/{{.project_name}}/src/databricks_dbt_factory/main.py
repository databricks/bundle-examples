...
def main():
    ...
    dbt_tasks_deps = (
        [item.strip() for item in args.dbt_tasks_deps.split(",") if item.strip()]
        if args.dbt_tasks_deps
        else []
    )
...
def _copy_runner_notebook(
    target_job_spec_path: str, project_directory: str | None
) -> tuple[str, bool]:
    """
    ...
    """
    source = (
        resources.files("databricks_dbt_factory")
        / "notebook"
        / _RUNNER_NOTEBOOK_FILENAME
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate Databricks job definition from dbt manifest."
    )
    parser.add_argument(
        "--new-job-name",
        type=str,
    ...
    parser.add_argument(
        "--dbt-manifest-path", type=str, help="Path to the manifest file", required=True
    )
    parser.add_argument(
        "--input-job-spec-path",
        type=str,
        help="Path to the input job spec file",
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--target", type=str, help="Optional dbt target to use.", required=False
    )
    parser.add_argument(
        "--source",
        type=str,
    ...
    parser.add_argument(
        "--warehouse_id",
        type=str,
        help="Optional SQL Warehouse to run dbt models on",
        required=False,
        default=None,
    )
    parser.add_argument(
        "--schema",
        type=str,
        help="Optional schema to write to.",
        required=False,
        default=None,
    )
    parser.add_argument(
        "--catalog",
        type=str,
        help="Optional catalog to write to.",
        required=False,
        default=None,
    )
    parser.add_argument(
