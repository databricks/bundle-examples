...
def get_pipeline_tags(wc: WorkspaceClient, pipeline_id: str) -> Optional[dict]:
    ...
    pipeline_spec = wc.api_client.do(
        method="get", path=f"/api/2.0/pipelines/{pipeline_id}"
    )["spec"]
...
def get_pipeline_ids_by_tags(
    ...
    if index_enabled and spark is not None and index_table_fqn:
        ...
        """).collect()[0]
            ...
            if age_hours is None or age_hours <= index_max_age_hours:
                ...
                log.info(
                    f"Using pipeline tags index table (age: {age_hours:.1f} hours)"
                )
                ...
                log.info(
                    f"Querying pipeline tags index table {index_table_fqn} with where conditions: {where_conditions}"
                )
        ...
        """
                ...
                for group in tag_groups:
                    ...
                    if group_pipeline_sets:
                        ...
                        if group_result:
                            log.info(
                                f"Found {len(group_result)} pipeline(s) matching group {group}"
                            )
    ...
    all_pipeline_ids = [
        (pi.pipeline_id, pi.name) for pi in wc.pipelines.list_pipelines()
    ]
        ...
        try:
            ...
            for group in tag_groups:
                ...
                for tag_key, tag_value in group:
                    if (
                        tag_key not in pipeline_tags
                        or pipeline_tags[tag_key] != tag_value
                    ):
                        group_matches = False
                        break
                ...
                if group_matches:
                    ...
                    log.info(
                        f"Pipeline {pipeline_name} ({pipeline_id}) matches group {group}"
                    )
...
class EventLogImporter:
    ...
    def __init__(
        self,
        monitoring_catalog: str,
        monitoring_schema: str,
        imported_event_logs_table: str,
    ...
    def create_target_table(self, spark: SparkSession):
        ...
        """
        spark.sql(
            f"CREATE TABLE IF NOT EXISTS {self.imported_event_logs_table_fqname} ({SDP_EVENT_LOG_SCHEMA}) CLUSTER BY AUTO"
        )
    ...
    def import_event_log_for_one_pipeline(self, pipeline_id: str, spark: SparkSession):
        ...
        self.log.info(
            f"Latest imported event for pipeline {pipeline_id} as of {latest_event_timestamp}"
        )

    def import_event_logs_for_pipelines(
        self, pipeline_ids: List[str], spark: SparkSession
    ):
        """
    ...
    def import_event_logs_for_pipelines_comma_list(
        self, pipeline_ids_list: str, spark: SparkSession
    ):
        """
        ...
        """
        self.import_event_logs_for_pipelines(
            parse_comma_separated_list(pipeline_ids_list), spark
        )

    def import_event_logs_for_pipelines_by_tags(
        self, tags_str: str, spark: SparkSession
    ):
        """
    ...
    def import_event_logs_for_pipelines_by_ids_and_tags(
        self, pipeline_ids_list: str, tags_str: str, spark: SparkSession
    ):
        """
        ...
        explicit_ids = (
            set(parse_comma_separated_list(pipeline_ids_list))
            if pipeline_ids_list
            else set()
        )
    ...
    def build_index(self, spark: SparkSession):
        ...
        for pipeline_id in all_pipelines_id:
            try:
                ...
                if processed_count % 100 == 0:
                    self.log.info(
                        f"Processed {processed_count}/{len(all_pipelines_id)} pipelines..."
                    )
        ...
        if tags_index:
            ...
            df.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(
                self.index_table_fqn
            )

            self.log.info(
                f"Successfully built pipeline tags index with {len(tags_index)} entries"
            )

        else:
            self.log.warning(
                "No tags found in any pipelines. Index table will not be created/updated."
            )
...
class DashboardTemplate:
    ...
    def __init__(
        ...
        self.wc = wc if wc else WorkspaceClient()
        self.dashboard_template_path = get_required_parameter(
            name="dashboard_template_path", value=dashboard_template_path
        )
        if not os.path.exists(dashboard_template_path):
            raise ValueError(
                f"Dashboard at path {dashboard_template_path} does not exist"
            )
        self.dashboard_id = get_optional_parameter(dashboard_id)
        self.published_dashboard_name = (
            published_dashboard_name
            if is_parameter_defined(published_dashboard_name)
            else self._extract_dashboard_name_from_path(dashboard_template_path)
        )
        ...
        self.warehouse_id = (
            warehouse_id
            if is_parameter_defined(warehouse_id)
            else self._get_default_warehouse_id()
        )
        if self.warehouse_id is None:
            raise Exception(
                "Unable to find a suitable warehouse for the AI/BI dashboard. Please set `warehouse_id` with the ID of the warehouse to use."
            )

    @staticmethod
    def from_notebook_widgets(widgets, wc: Optional[WorkspaceClient] = None):
        return DashboardTemplate(
            dashboard_template_path=widgets.get("dashboard_template_path"),
    ...
    def _get_default_warehouse_id(self):
        ...
        if preferred_warehouse is not None:
            self.log.info(
                f"Using warehouse: {preferred_warehouse.name} ({preferred_warehouse.id})"
            )
    ...
    def _find_all_dashboards_with_name(self, display_name: str):
        ...
        for d in self.wc.lakeview.list():
            if d.display_name == display_name:
                ...
                self.log.info(
                    f"Found existing dashboard with display name '{d.display_name}' ({d.dashboard_id})"
                )
            else:
                self.log.debug(
                    f"Ignoring dashboard with display name '{d.display_name}' != '{display_name}'"
                )
    ...
    def _get_dashboard_id(self) -> Optional[str]:
        ...
        candidate_ids = self._find_all_dashboards_with_name(
            self.published_dashboard_name
        )
        if len(candidate_ids) > 1:
            raise ValueError(
                f"Multiple dashboard found with display name {self.published_dashboard_name}. Please specify an explicit `dashboard_id`."
            )
    ...
    def publish(self):
        ...
        if real_dashboard_id is None:
            self.log.info(
                f"Creating new dashboard with display name '{self.published_dashboard_name}'"
            )
        else:
        ...
        if real_dashboard_id is None:
            ...
            self.log.info(
                f"Created dashboard '{d.display_name}' (ID={d.dashboard_id} ETAG={d.etag})"
            )
            real_dashboard_id = d.dashboard_id
        else:
            d_json["dashboard_id"] = real_dashboard_id
            d = self.wc.lakeview.update(
                dashboard_id=real_dashboard_id, dashboard=Dashboard.from_dict(d_json)
            )
            self.log.info(
                f"Updated dashboard '{d.display_name}' (ID={d.dashboard_id} ETAG={d.etag})"
            )

        pd = self.wc.lakeview.publish(
            dashboard_id=real_dashboard_id,
            embed_credentials=True,
            warehouse_id=self.warehouse_id,
        )
        self.log.info(
            f"Published dashboard '{pd.display_name}' revision time {pd.revision_create_time}"
        )
