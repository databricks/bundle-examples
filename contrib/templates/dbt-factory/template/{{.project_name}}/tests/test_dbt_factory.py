...
def _test(
    package: str, name: str, depends_on: list[str], severity: str = "error"
) -> tuple[str, dict]:
...
def test_same_model_name_across_packages_produces_distinct_bundled_test_tasks(
    dbt_factory_bundled,
):
    nodes = dict(
        [
            ...
            _model(
                "pkg_a",
                "orders",
                depends_on=["model.pkg_a.customers", "model.pkg_b.customers"],
            ),
    ...
    assert by_key["tests_model_pkg_a_customers"]["depends_on"] == [
        {"task_key": "model_pkg_a_customers"}
    ]
    assert by_key["tests_model_pkg_b_customers"]["depends_on"] == [
        {"task_key": "model_pkg_b_customers"}
    ]
...
def test_tests_on_seed_produce_task_and_gate_downstream(dbt_factory_bundled):
    ...
    assert by_key["tests_seed_pkg_countries"]["depends_on"] == [
        {"task_key": "seed_pkg_countries"}
    ]
    assert by_key["model_pkg_enriched"]["depends_on"] == [
        {"task_key": "tests_seed_pkg_countries"}
    ]
...
def test_tests_on_snapshot_produce_task_and_gate_downstream(dbt_factory_bundled):
    ...
    assert by_key["tests_snapshot_pkg_orders_snap"]["depends_on"] == [
        {"task_key": "snapshot_pkg_orders_snap"}
    ]
    assert by_key["model_pkg_orders_history"]["depends_on"] == [
        {"task_key": "tests_snapshot_pkg_orders_snap"}
    ]
...
def test_flat_mode_emits_one_task_per_test_node_and_gates_downstream(dbt_factory):
    ...
    assert by_key["test_pkg_unique_customers_id"]["depends_on"] == [
        {"task_key": "model_pkg_customers"}
    ]
...
def test_flat_mode_cross_model_test_does_not_create_cycle(dbt_factory):
    ...
    payments_deps = {
        dep["task_key"] for dep in by_key["model_pkg_payments"]["depends_on"]
    }
...
def test_flat_mode_test_on_seed_gates_on_seed(dbt_factory):
    ...
    assert by_key["test_pkg_unique_countries_code"]["depends_on"] == [
        {"task_key": "seed_pkg_countries"}
    ]
...
def test_bundled_task_factory_assembles_commands(dbt_factory_bundled):
    ...
    assert task.commands == [
        "dbt test --select pkg.customers --indirect-selection cautious --target dev"
    ]
...
def test_cross_model_test_in_bundled_mode_is_emitted_as_standalone_task(
    ...
    cross_test_key = (
        "test_pkg_relationships_game_details_winner__team_city__ref_team_cities_"
    )
...
def test_single_package_bundled_test_uses_qualified_select(dbt_factory_bundled):
    ...
    assert by_key["model_pkg_a_orders"]["depends_on"] == [
        {"task_key": "tests_model_pkg_a_customers"}
    ]
...
def test_create_job_spec_and_update_with_selected_dbt_deps(
...
def test_generate(databricks_dbt_factory):
    ...
    databricks_dbt_factory.create_tasks_and_update_job_spec(
        dbt_manifest_path,
        job_definition_path,
        destination_job_definition_path,
        "new_job_name",
    )
