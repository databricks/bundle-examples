...
def fetch_access_token(config):
    ...
    try:
        response = requests.post(
            token_url, data=payload, timeout=config["request_timeout_sec"]
        )
        ...
        return {"access_token": token_json["access_token"], "last_load_timestamp": now}
...
def getThirdPartySinkConfigFromSparkConfig(spark_conf):
    ...
    common_params = {
        ...
        "max_access_token_staleness": int(
            spark_conf.get("azure_max_access_token_staleness", "3300")
        ),
...
class AzureMonitorMetricsConverter:
    ...
    def create_http_requests_spec(
        ...
        return (
            ...
            .withColumn(
                "payload",
                concat(lit("["), expr("concat_ws(',', batch_metrics)"), lit("]")),
            )
...
token_details = {"access_token": None, "last_load_timestamp": 0}
...
def get_access_token(config):
    ...
    now = int(datetime.now(timezone.utc).timestamp())
    if (now - token_details["last_load_timestamp"]) < config[
        "max_access_token_staleness"
    ]:
    ...
    with token_lock:
        if (
            token_details["access_token"] is None
            or (now - token_details["last_load_timestamp"])
            >= config["max_access_token_staleness"]
        ):
